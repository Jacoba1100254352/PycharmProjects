...
def write_updated_data_to_csv(filename, data):
    ...
    """
    columns = ["Time [s]", "ADC", "Force [N]"] if SIMPLIFY else ["Time [s]", "ADC1", "ADC2", "ADC3", "ADC4", "Force1 [N]", "Force2 [N]", "Force3 [N]", "Force4 [N]", "TotalForce1 [N]", "TotalForce2 [N]"]
...
def apply_calibration_coefficients():
    ...
    for sensor_num in range(1, NUM_SENSORS + 1):
        ...
        if SIMPLIFY:
            aligned_arduino_data["Force [N]"] = round(new_coefficients[0][0] * aligned_arduino_data["ADC"] + new_coefficients[0][1], 2)
        else:
            for j in range(4):
                aligned_arduino_data[f"Force{j + 1} [N]"] = round(new_coefficients[j][0] * aligned_arduino_data[f"ADC{j + 1}"] + new_coefficients[j][1], 2)
        ...
        updated_csv_filename = CALIBRATED_DIR / f"Updated Calibration Test {TEST_NUM} Sensor {sensor_num} Data.csv"
