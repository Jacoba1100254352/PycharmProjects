...
def align_and_save_arduino_data(sensor_num, instron_data, arduino_data, aligned_data_dir):
    """
    ...
    instron_index, _ = find_percentile_index(instron_data["Force [N]"])
    arduino_index, _ = find_percentile_index(arduino_data[f"ADC{'' if SIMPLIFY else sensor_num}"])
